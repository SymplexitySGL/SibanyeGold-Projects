...
ALTER PROCEDURE [dbo].[sp_Group_Transfer_Employee_Cook]
/*-------------------------------------------------------------------------------------
*  CONFIGURATION CONTROL
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
...
AS 
    DECLARE @sMsg VARCHAR(4000) ,
		...
		AND GDC.[End Date] = '99991231'
		 ...
		 SET @MovementType = 'Re-Engagement'
...
--- Create Personal employment details 
    ...
    BEGIN TRY
	   ...
	   SET @PIOpen = (SELECT  TOP 1 [Period ID] FROM [dbo].[Calendar Periods] WHERE (GETDATE() BETWEEN [Start Date] AND [End Date] OR [Start Date] > @Date ) AND [Completed] = 'No' AND RunType = 'Normal' AND [Calendar] = @Cycle ORDER BY [Period ID] ASC)
        ...
        VALUES  ( @ResourceTag , -- Resource Tag - int
                 ...
                 @MovementType , -- Movement Type - nvarchar(50)
                  ...
                  'sp_Group_Transfer_Employee'
	    			...
	    			EXEC 	sp_Emp_TA_movement @ResourceTag,@Date,@MovementType,'',@Designation,'G0395193','Employment Movement',@Cycle
...
--- Coooke Special Section Section


DELETE FROM [dbo].[Input Transactions] WHERE [Resource Tag] = @ResourceTag

INSERT INTO [dbo].[Input Transactions]
        ( [Resource Tag] ,
          [Element] ,
          [Period ID] ,
          [Back Pay] ,
...
VALUES  ( @ResourceTag , -- Resource Tag - int
          N'TA Date Start (Input)' , -- Element - nvarchar(50)
          @PIOpen , -- Period ID - int
          N'No' , -- Back Pay - nvarchar(50)
          LEFT(CAST(@StartingPeriodID AS NVARCHAR(20)),8) , -- Input Value - decimal
          N'SYM' , -- User ID - nvarchar(50)
          N'Cook Re-engagement' , -- Source - nvarchar(50)
          GETDATE() 
        )
...
SET @ScriptName = 'SYM39546-02 PK Cooke GT Proc Change.sql'
