...
ALTER PROCEDURE [dbo].[sp_Group_Transfer_Employee]
/*-------------------------------------------------------------------------------------
*  CONFIGURATION CONTROL
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
...
--- Create Personal employment details 
    ...
    BEGIN TRY
        ...
        IF @StartingPeriodIDNew IS NOT NULL
            BEGIN
                SET @StartingPeriodID = ( SELECT TOP 1
                                                    [Period ID]
                                          FROM      [dbo].[Calendar Periods]
                                          WHERE     [Period ID] > @StartingPeriodIDNew
                                                    AND Calendar = @Cycle
                                          ORDER BY  [Sequence]
                                        )
            END	      
        ...
        VALUES  ( @ResourceTag , -- Resource Tag - int
                  ...
                  'Transer SYM Dec 2011' , -- Comments - nvarchar(100)
...
---- Work History ---
        ...
        VALUES  ( @ResourceTag , -- Resource Tag - int
                  ...
                  'Re-Engagement' , -- Movement Type - nvarchar(50)
...
---Emp Retirement Fund
        ...
        SET @RemMethod = ( SELECT   [To Remuneration Method]
                           FROM     [dbo].[Emp Work History]
                           WHERE    [Resource Tag] = @ResourceTag
                                    AND [End Date] = '99991231'
                         )
            ...
            BEGIN
                ...
                SET @DefaultFund = ( SELECT TOP 1
                                     ...
                                     WHERE  [Operation] = @FromOperation
                                            AND @Date BETWEEN [Start Date] AND [End Date]
                                            AND [Remuneration Method] = @RemMethod
                                   )
...
SET @ScriptName = 'SYM39546-04 PK Pro GT Proc Change.sql'
